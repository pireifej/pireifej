(function () {
    ...
    function playAlarm() {
        try {
            var context = getAudioContext();
            var frequencies = [880, 0, 880, 0, 880, 0, 1100, 0, 1100];
            var duration = 0.15;
            frequencies.forEach(function (frequency, index) {
                ...
                gain.gain.setValueAtTime(0.3, context.currentTime + index * duration);
                gain.gain.exponentialRampToValueAtTime(0.01, context.currentTime + (index + 1) * duration);
    ...
    function updateTimerDisplay() {
        ...
        if (timeLeft > 45) circle.classList.add('timer-green');
        else if (timeLeft > 20) circle.classList.add('timer-yellow');
    ...
    function startTimer() {
        ...
        try { getAudioContext(); } catch (error) {}
    ...
    function resetTimer() {
        ...
        timeLeft = 90;
        updateTimerDisplay();
        document.getElementById('timer-circle').className = 'timer-circle timer-green';
        document.getElementById('timer-start').disabled = false;
    }
