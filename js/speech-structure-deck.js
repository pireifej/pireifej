(function () {
    ...
    function updateTimerDisplay() {
        ...
        display.textContent = Math.floor(timeLeft / 60) + ':' + (timeLeft % 60 < 10 ? '0' : '') + timeLeft % 60;
        circle.classList.remove('timer-green', 'timer-yellow', 'timer-red');
        circle.classList.add(timeLeft > 30 ? 'timer-green' : timeLeft > 15 ? 'timer-yellow' : 'timer-red');
    }
    function resetTimer() {
    ...
    function startTimer() {
        ...
        timerInterval = setInterval(function () {
            timeLeft -= 1;
            updateTimerDisplay();
            if (timeLeft === 15 || timeLeft === 10 || timeLeft === 5) beep(660, 0.12);
            if (timeLeft <= 0) {
                clearInterval(timerInterval);
                timerInterval = null;
                ...
                [880, 880, 880, 1100, 1100].forEach(function (frequency, index) {
                    window.setTimeout(function () { beep(frequency, 0.14); }, index * 190);
                });
            }
        }, 1000);
    }
    ...
    document.getElementById('timer-start').addEventListener('click', function (event) { event.stopPropagation(); startTimer(); });
    document.getElementById('timer-reset').addEventListener('click', function (event) { event.stopPropagation(); resetTimer(); });
